    ...
    def __init__(self, scope, globalAC=None):
        ...
        if scope == GLOBAL_NET_SCOPE:  # create global network
            ...
            with tf.variable_scope(scope):
                ...
                # self.c1_CR_d = tf.nn.fixed_unigram_candidate_sampler(true_classes=lib.CR_mapping_index, num_true=40,
                #                                                          num_sampled=1, unique=False, range_max=40,
                #                                                          unigrams=self.CR1_prob[0])
                self.c1_CR_d = tf.argmax(self.CR1_prob)
                self.c1_CR = lib.CR_mapping[self.c1_CR_d[0]]

                self.c2_CR_d = tf.argmax(self.CR2_prob)
                self.c2_CR = lib.CR_mapping[self.c2_CR_d[0]]

                self.c3_CR_d = tf.argmax(self.CR3_prob)
                self.c3_CR = lib.CR_mapping[self.c3_CR_d[0]]

                self.c4_CR_d = tf.argmax(self.CR4_prob)
                self.c4_CR = lib.CR_mapping[self.c4_CR_d[0]]


                self.capa2_all = options.serverCC - lib.CR_mapping[self.c1_CR_d][0] * options.serverCC
                self.capa3_all = self.capa2_all - lib.CR_mapping[self.c2_CR_d][0] * self.capa2_all
                self.capa4_all = self.capa3_all - lib.CR_mapping[self.c3_CR_d][0] * self.capa3_all

                self.capa1_prob = lib.CR_mapping[self.c1_CR_d][0]
                self.capa2_prob = (lib.CR_mapping[self.c2_CR_d][0] * self.capa2_all) / options.serverCC
                self.capa3_prob = (lib.CR_mapping[self.c3_CR_d][0] * self.capa3_all) / options.serverCC
                self.capa4_prob = (lib.CR_mapping[self.c4_CR_d][0] * self.capa4_all) / options.serverCC
                ...
                with tf.name_scope('CR_A_loss'):
                    self.CR1_log = tf.log(self.capa1_prob + 1e-2)
                    self.CR1_hot = tf.one_hot(self.cr1_a, 40, dtype=tf.float32)
                    self.CR1_log_hot = tf.log(self.capa1_prob + 1e-2) * tf.one_hot(self.cr1_a, 40, dtype=tf.float32)
                    self.CR1_log_prob = tf.reduce_sum(
                        tf.log(self.capa1_prob + 1e-2) * tf.one_hot(self.cr1_a, 40, dtype=tf.float32), axis=1)

                    self.CR1_exp_v = self.CR1_log_prob * tf.stop_gradient(self.CR1_td)
                    self.CR1_entropy = - tf.reduce_sum(self.capa1_prob * tf.log(self.capa1_prob + 1e-2), axis=1)
                ...
                with tf.name_scope('CR_A_loss'):
                    self.CR2_log = tf.log(self.capa2_prob + 1e-2)
                    self.CR2_hot = tf.one_hot(self.cr2_a, 40, dtype=tf.float32)
                    self.CR2_log_hot = tf.log(self.capa2_prob + 1e-2) * tf.one_hot(self.cr2_a, 40, dtype=tf.float32)
                    self.CR2_log_prob = tf.reduce_sum(
                        tf.log(self.capa2_prob + 1e-2) * tf.one_hot(self.cr2_a, 40, dtype=tf.float32), axis=1)

                    self.CR2_exp_v = self.CR2_log_prob * tf.stop_gradient(self.CR2_td)
                    self.CR2_entropy = - tf.reduce_sum(self.capa2_prob * tf.log(self.capa2_prob + 1e-2), axis=1)
                ...
                with tf.name_scope('CR_A_loss'):
                    self.CR3_log = tf.log(self.capa3_prob + 1e-2)
                    self.CR3_hot = tf.one_hot(self.cr3_a, 40, dtype=tf.float32)
                    self.CR3_log_hot = tf.log(self.capa3_prob + 1e-2) * tf.one_hot(self.cr3_a, 40, dtype=tf.float32)
                    self.CR3_log_prob = tf.reduce_sum(
                        tf.log(self.capa3_prob + 1e-2) * tf.one_hot(self.cr3_a, 40, dtype=tf.float32), axis=1)

                    self.CR3_exp_v = self.CR3_log_prob * tf.stop_gradient(self.CR3_td)
                    self.CR3_entropy = - tf.reduce_sum(self.capa3_prob * tf.log(self.capa3_prob + 1e-2), axis=1)
                ...
                with tf.name_scope('CR_A_loss'):
                    self.CR4_log = tf.log(self.capa4_prob + 1e-2)
                    self.CR4_hot = tf.one_hot(self.cr4_a, 40, dtype=tf.float32)
                    self.CR4_log_hot = tf.log(self.capa4_prob + 1e-2) * tf.one_hot(self.cr4_a, 40, dtype=tf.float32)
                    self.CR4_log_prob = tf.reduce_sum(
                        tf.log(self.capa4_prob + 1e-2) * tf.one_hot(self.cr4_a, 40, dtype=tf.float32), axis=1)

                    self.CR4_exp_v = self.CR4_log_prob * tf.stop_gradient(self.CR4_td)
                    self.CR4_entropy = - tf.reduce_sum(self.capa4_prob * tf.log(self.capa4_prob + 1e-2), axis=1)
    ...
    def work(self):
        ...
        while GLOBAL_EP < MAX_GLOBAL_EP:
            ...
            while True:
                ...
                c1_CRList = self.AC.c1_CR
                c1_CRList_d = self.AC.c1_CR_d
                c2_CRList = self.AC.c2_CR
                c2_CRList_d = self.AC.c2_CR_d
                c3_CRList = self.AC.c3_CR
                c3_CRList_d = self.AC.c3_CR_d
                c4_CRList = self.AC.c4_CR
                c4_CRList_d = self.AC.c4_CR_d
